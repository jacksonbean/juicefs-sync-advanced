...
func recordFlags() []cli.Flag {
	return addCategories("RECORD", []cli.Flag{
		...
		&cli.StringFlag{
			Name:  "record-db-dsn",
			Usage: "database DSN for recording sync operations",
		},
		&cli.StringFlag{
			Name:  "record-table",
			Value: "objects",
			Usage: "table name for recording sync operations",
		},
		&cli.StringFlag{
			Name:  "record-plan-table",
			Usage: "table name for sync_plan records (used during dry-run)",
		},
		&cli.BoolFlag{
			Name:  "record-extended-fields",
			Usage: "enable extended fields (md5, retention, etc.) in sync records",
		},
		&cli.StringFlag{
			Name:  "record-run-id",
...
func doSync(c *cli.Context) error {
	...
	// Build config JSON for history storage
	if config.RecordDBDSN != "" {
		cfgMap := map[string]interface{}{
			...
			"force_update":  c.Bool("force-update"),
			"delete_src":    c.Bool("delete-src"),
			"delete_dst":    c.Bool("delete-dst"),
			"check_all":     c.Bool("check-all"),
			"check_new":     c.Bool("check-new"),
			"perms":         c.Bool("perms"),
			"bwlimit":       c.Int("bwlimit"),
			"instance_name": c.String("instance-name"),
		}
		...
		if incR := c.StringSlice("include-regex"); len(incR) > 0 {
			cfgMap["include_regex"] = incR
		}
		if excR := c.StringSlice("exclude-regex"); len(excR) > 0 {
			cfgMap["exclude_regex"] = excR
		}
		if c.IsSet("start-time") {
			cfgMap["start_time"] = c.String("start-time")
		}
		if c.IsSet("end-time") {
			cfgMap["end_time"] = c.String("end-time")
		}
		if c.IsSet("max-size") {
			cfgMap["max_size"] = c.String("max-size")
		}
		if c.IsSet("min-size") {
			cfgMap["min_size"] = c.String("min-size")
		}
		if c.IsSet("files-from") {
			cfgMap["files_from"] = c.String("files-from")
		}
		if b, err := json.Marshal(cfgMap); err == nil {
			config.ConfigJSON = string(b)
		}
	}

	// Mask password in record-db-dsn from process title
	if config.RecordDBDSN != "" {
		if dsn2 := utils.RemovePassword(config.RecordDBDSN); dsn2 != config.RecordDBDSN {
			for i := range os.Args {
				if os.Args[i] == config.RecordDBDSN {
					os.Args[i] = dsn2
				} else if strings.HasPrefix(os.Args[i], "--record-db-dsn=") {
					os.Args[i] = "--record-db-dsn=" + dsn2
				}
			}
		}
	}
	...
	if config.Manager == "" {
		...
		// Clean up instance registry on normal completion (signals handled inside exposeMetrics)
		defer unregisterInstance(os.Getpid())
		if c.IsSet("consul") {
