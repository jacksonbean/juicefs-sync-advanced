...
func updateStats(r *Stat, config *Config) {
	state := getState(config)
	state.copied.IncrInt64(r.Copied)
	state.copiedBytes.IncrInt64(r.CopiedBytes)
	if state.checked != nil {
		state.checked.IncrInt64(r.Checked)
		state.checkedBytes.IncrInt64(r.CheckedBytes)
	}
	if state.deleted != nil {
		state.deleted.IncrInt64(r.Deleted)
	}
	state.skipped.IncrInt64(r.Skipped)
	state.skippedBytes.IncrInt64(r.SkippedBytes)
	if state.failed != nil {
		state.failed.IncrInt64(r.Failed)
	}
	state.handled.IncrInt64(r.Copied + r.Deleted + r.Skipped + r.Failed)
}
...
func sendStats(addr string, config *Config) {
	state := getState(config)
	sendStatMu.Lock()
	defer sendStatMu.Unlock()
	var r Stat
	r.Skipped = state.skipped.Current()
	r.SkippedBytes = state.skippedBytes.Current()
	r.Copied = state.copied.Current()
	r.CopiedBytes = state.copiedBytes.Current()
	srcDelayDelMu.Lock()
	r.DelayDelDir = srcDelayDel
	srcDelayDel = make([]string, 0)
	srcDelayDelMu.Unlock()
	if state.checked != nil {
		r.Checked = state.checked.Current()
		r.CheckedBytes = state.checkedBytes.Current()
	}
	if state.deleted != nil {
		r.Deleted = state.deleted.Current()
	}
	if state.failed != nil {
		r.Failed = state.failed.Current()
	}
	d, _ := json.Marshal(r)
