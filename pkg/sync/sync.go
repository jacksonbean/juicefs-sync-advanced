...
func try(n int, f func() error, onRetry ...func(int, error)) (err error) {
	for i := 0; i < n; i++ {
		err = f()
		if !shouldRetry(err) {
			return
	...
	for {
		...
		case markChecksum:
			...
			if equal, err := checkSum(src, dst, key, nil, obj, config); err != nil {
				...
				if shouldVerify && config.RecordCallback != nil {
		...
		default:
			...
			if config.RecordCallback != nil {
				config.RecordCallback.OnTransferStart(key, key, obj.IsDir(), obj.Size(), obj.Mtime())
			}
			var err error
			var srcChksum uint32
			...
			if err == nil {
				...
				if (config.CheckAll || config.CheckNew) && config.RecordCallback != nil {
					config.RecordCallback.OnVerifyComplete(key, key, obj.Size(), obj.Mtime())
				}
...
func handleExtraObject(tasks chan<- object.Object, dstobj object.Object, config *Config) bool {
	...
	if config.Dry && config.RecordCallback != nil {
		config.RecordCallback.OnPlanned(dstobj.Key(), dstobj.Key(), dstobj.Size(), dstobj.Mtime(), "DeleteDst", "Extra")
	}
	config.Limit--
	if dstobj.IsDir() {
		dstDelayDelMu.Lock()
...
func produce(tasks chan<- object.Object, srckeys, dstkeys <-chan object.Object, config *Config) error {
	...
	for obj := range srckeys {
		...
		if dstobj == nil || obj.Key() < dstobj.Key() {
			if config.Existing {
				if config.Dry && config.RecordCallback != nil {
					config.RecordCallback.OnPlanned(obj.Key(), "", obj.Size(), obj.Mtime(), "Skip", "Existing")
				}
				skipIt(obj)
				continue
			}
			if config.Dry && config.RecordCallback != nil {
				config.RecordCallback.OnPlanned(obj.Key(), "", obj.Size(), obj.Mtime(), "Copy", "NotExist")
			}
			tasks <- obj
		} else { // obj.key == dstobj.key
			if config.IgnoreExisting {
				if config.Dry && config.RecordCallback != nil {
					config.RecordCallback.OnPlanned(obj.Key(), dstobj.Key(), obj.Size(), obj.Mtime(), "Skip", "IgnoreExisting")
				}
				skipIt(obj)
				dstobj = nil
				continue
			...
			if config.ForceUpdate ||
				...
				if !config.ForceUpdate {
					if config.Update {
						...
						reason = "DifferentSize"
					}
				}
				if config.Dry && config.RecordCallback != nil {
					config.RecordCallback.OnPlanned(obj.Key(), dstobj.Key(), obj.Size(), obj.Mtime(), "Update", reason)
				}
				tasks <- obj
			} else if config.Update && obj.Mtime().Unix() < dstobj.Mtime().Unix() {
				if config.Dry && config.RecordCallback != nil {
					config.RecordCallback.OnPlanned(obj.Key(), dstobj.Key(), obj.Size(), obj.Mtime(), "Skip", "Older")
				}
				skipIt(obj)
			} else if config.CheckAll { // two objects are likely the same
				if config.Dry && config.RecordCallback != nil {
					config.RecordCallback.OnPlanned(obj.Key(), dstobj.Key(), obj.Size(), obj.Mtime(), "Checksum", "SameSizeTime")
				}
				tasks <- withSize(obj, markChecksum)
			} else if config.DeleteSrc {
				if config.Dry && config.RecordCallback != nil {
					config.RecordCallback.OnPlanned(obj.Key(), dstobj.Key(), obj.Size(), obj.Mtime(), "DeleteSrc", "Same")
				}
				if obj.IsDir() {
					srcDelayDelMu.Lock()
				...
				if config.Dry && config.RecordCallback != nil {
					config.RecordCallback.OnPlanned(obj.Key(), dstobj.Key(), obj.Size(), obj.Mtime(), "CopyPerms", "PermsDiff")
				}
				tasks <- withSize(obj, markCopyPerms)
			} else {
				if config.Dry && config.RecordCallback != nil {
					config.RecordCallback.OnPlanned(obj.Key(), dstobj.Key(), obj.Size(), obj.Mtime(), "Skip", "Same")
				}
				skipIt(obj)
			}
			dstobj = nil
...
func Sync(parentCtx context.Context, src, dst object.ObjectStorage, config *Config) error {
	...
	if config.RecordDBType != "" && config.RecordDBDSN != "" {
		recorder, err := record.NewRecorder(config.RecordDBType, config.RecordDBDSN, &record.ManagerConfig{
			...
			PlanTableName:         config.RecordPlanTableName,
			ExtendedFieldsEnabled: config.RecordExtendedFields,
			BatchSize:             config.RecordBatchSize,
			FlushInterval:         config.RecordFlushInterval,
			MaxOpenConns:          10,
			MaxIdleConns:          5,
			DryRun:                config.Dry,
		})
		...
		if config.RecordRunID == "" {
			prefix := "sync"
			if config.Dry {
				prefix = "dryrun"
			}
			config.RecordRunID = fmt.Sprintf("%s-%d", prefix, time.Now().UnixNano())
		}
		config.RecordCallback = &record.Callback{
			Recorder:              recorder,
			ExtendedFieldsEnabled: config.RecordExtendedFields,
