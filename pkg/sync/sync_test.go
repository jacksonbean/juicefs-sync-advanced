...
func TestSync(t *testing.T) {
	...
	// Copy a2
	if err := Sync(context.Background(), a, b, config); err != nil {
		t.Fatalf("sync: %s", err)
	}
	if c := getState(config).copied.Current(); c != 1 {
	...
	// No copy occurred
	if c := getState(config).copied.Current(); c != 0 {
	...
	config.Exclude = nil
	config.rules = nil
	if err := Sync(context.Background(), b, a, config); err != nil {
		t.Fatalf("sync: %s", err)
	}
	if c := getState(config).copied.Current(); c != 1 {
		t.Fatalf("should copy 1 keys, but got %d", c)
...
func TestSyncLink(t *testing.T) {
	...
	if err := Sync(context.Background(), a, b, &Config{
		Threads:     50,
		Update:      true,
		Perms:       true,
		ListThreads: 1,
...
func TestLimits(t *testing.T) {
	...
	for _, tcase := range testCases {
		...
		if err := Sync(context.Background(), a, tcase.dst, config); err != nil {
			t.Fatalf("sync: %s", err)
		}
